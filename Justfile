...
compliance-check:
    ...
    umask 0022
    {
      ...
      echo "# Scope: union of go list -deps for ./cmd/aicli ./cmd/pingcode ./cmd/fns ./cmd/ozon"
      echo
      for pkg in ./cmd/aicli ./cmd/pingcode ./cmd/fns ./cmd/ozon; do
        echo "## $pkg"
...
build: build-pingcode build-fns build-ozon
...
release-checksums: _validate-version
    ...
    if [[ "$archive_count" -ne 24 ]]; then
      echo "expected 24 release archives, found ${archive_count}" >&2
      exit 1
    fi
...
_build-archive command goos goarch:
    ...
    if [[ "$command" == "pingcode" || "$command" == "fns" || "$command" == "ozon" ]]; then
